...
function resizeCanvas() {
  ...
  W = width;
  H = height;
  if(typeof spawnStars === 'function') spawnStars();
  if(typeof miniMapElem !== 'undefined')
    miniMapElem.style.display = (width > 800) ? '' : 'none';
}
resizeCanvas();
...
function resetGame() {
  ...
  miniMapElem.style.display = (W > 800) ? '' : 'none';
  spawnStars();
  updateHeartsUI();
}
function spawnStars() {
...
function drawMiniMap() {
  if (W<=800) { miniMapElem.style.display='none'; return; }
