...
// Firebase is initialized in index.html with CDN, so use firebase global
...
// -- Auth UI Logic (Sign Up / Sign In / Sign Out) --
...
function showUserPanel(data) {
  ...
  userPic.src = data.photoURL || 'https://ui-avatars.com/api/?name=' + encodeURIComponent(data.displayName||"User");
  ...
  if (user) {
    ...
    if (!doc.exists) {
      await db.collection("users").doc(user.uid).set({
        displayName: user.displayName || authName.value || user.email.split("@")[0],
        email: user.email,
...
signUpBtn.onclick = async () => {
  ...
  try {
    ...
    let photoURL = '';
    if (authPic.files && authPic.files[0]) {
      const snap = await storage.ref('profiles/'+user.uid).put(authPic.files[0]);
      photoURL = await snap.ref.getDownloadURL();
      await user.updateProfile({photoURL});
    }
    await user.updateProfile({ displayName: authName.value, photoURL });
    await db.collection("users").doc(user.uid).set({
...
function updateUserData(k, v) {
  if (!currentUser) return;
  userData[k] = v;
  db.collection("users").doc(currentUser.uid).update({ [k]: v });
}
function saveGameStats(stats) {
  if (!currentUser) return;
  db.collection("users").doc(currentUser.uid).update(stats);
}
...
document.getElementById('upgradeBtn').onclick = ()=>{
  ...
  if(coins >= 14 && upgrades.weapon < 10 && upgrades.upgrades < 10) {
    ...
    updateUserData('weaponLevel', upgrades.weapon);
    updateUserData('upgrades', upgrades.upgrades);
    updateUserData('coins', coins);
    playSound('upgrade');
  }
};
...
document.getElementById('slowmoBtn').onclick = ()=>{
  if(coins >= 20 && slowmoTimer <= 0) {
    coins -= 20; slowmoTimer = 320; updateUserData('coins', coins);
    playSound('powerup');
  }
};
...
function gameLoop() {
  ...
  if (gameState === 'paused' || !currentUser) return;
  ...
  if (gameState === 'playing') {
    ...
    for (let p of powerups) {
      if (dist(player, p) < player.r+p.r) {
        if (p.type==='coin') { coins+=2+Math.floor(level/2); updateUserData('coins', coins);}
        ...
        if (p.type==='double' && upgrades.weapon < 10 && upgrades.upgrades < 10) {
          upgrades.weapon++; upgrades.upgrades++; updateUserData('weaponLevel', upgrades.weapon); updateUserData('upgrades', upgrades.upgrades); playSound('upgrade');
        }
    ...
    for (let e of enemies) {
      ...
      if (dist(e, player) < e.r + player.r) {
        if (player.shield>0) {
          ...
          e.x = -999;
          if (player.hp <= 0) {
            if(score > best) { best = score; updateUserData('best', best);}
            updateUserData('coins', coins);
            updateUserData('hearts', player.hp);
            showGameOver();
          }
        }
      }
  ...
  if (gameState === 'menu') {
    ...
    ctx.fillStyle = '#fff';
    ctx.fillText('Sign in to play!', W/2, H/2-30);
    ctx.font = '18px VT323, Courier New, monospace';
    ctx.fillStyle = '#0ff';
    ctx.fillText(`Best: ${best}   Coins: ${coins}`, W/2, H/2+10);
...
function showGameOver() {
  ...
  ctx.font = '18px VT323, Courier New, monospace';
  ctx.fillStyle = '#0ff';
  ctx.fillText(`Coins: ${coins}   Best: ${best}`, W/2, H/2+64);
  ctx.fillStyle = '#fff9';
