// ==== VOID LOOP: Nakama Pro+ Edition ==== //
// Fully synced for index.html v6 and style.css v5
// All UI buttons function, all features present, mobile/desktop/touch, minimap, bomb, music, bullet synth, and smooth!
...
function persist() { localStorage.voidloopSave = JSON.stringify(save); }
function statInc(k, amt=1) { save[k]=(save[k]||0)+amt; persist(); }
...
function resetGame() {
  ...
  bombCount = save.bombs||1; timeSurvived = 0; bombCooldown = 0;
  upgrades = { weapon: save.weapon||1, health: save.health||3 };
  player = { x: W/2, y: H*0.8, r: 22, cooldown: 0, hp: upgrades.health, vx: 0, vy: 0, angle: 0, shield: 0 };
...
function gameLoop() {
  ...
  if (gameState === 'playing') {
    timeSurvived += 1/60;
    // Player follows mouse/touch always
